...
class MarketBudgetChart extends Component {
  ...
  editBudget = (id, categoryId, budget, quantity) => {
    this.setState({
      ...
      this.setState({ createMarketBudget: true })
    });
  }
      ...
      [
        {
          ...
          onPress: async () => {
            ...
            const marketId = this.props.navigation.getParam("item").id;
            await axios.get(`${Api.url}/${userId}/budget/market/${marketId}/delete/${id}`, { headers: Api.headers }).then((response) => {
  ...
  fetchBudgets = async () => {
    ...
    axios.get(`${Api.url}/${userId}/budget/market/chart/${id}`, { headers: Api.headers }).then(async (response) => {
  ...
  renderModalChild = () => {
    if (this.state.modalFor === MODALFORCREATE) {
      return (
        <CreateMarketBudget
          isAvailable={true}
          eventId={this.props.navigation.getParam("item").id}
          closeModal={(isAdded) => {
...
export default MarketBudgetChart;
